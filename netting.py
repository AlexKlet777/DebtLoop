...
def calculate_cycle_netting(cycle: List[Debt]) -> Dict[str, Any]:
    ...
    netting_amount = min(int(d["amount"]) for d in cycle)
    total_before = sum(int(d["amount"]) for d in cycle)

    residual_debts = []
    for debt in cycle:
        residual = int(debt["amount"]) - netting_amount
        residual_debts.append(
            {
                "from": debt["from"],
                "to": debt["to"],
                "before": int(debt["amount"]),
                "after": residual,
            }
        )
